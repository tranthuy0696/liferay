/**
 * Copyright (c) 2000-present Liferay, Inc. All rights reserved.
 *
 * This library is free software; you can redistribute it and/or modify it under
 * the terms of the GNU Lesser General Public License as published by the Free
 * Software Foundation; either version 2.1 of the License, or (at your option)
 * any later version.
 *
 * This library is distributed in the hope that it will be useful, but WITHOUT
 * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 * FOR A PARTICULAR PURPOSE. See the GNU Lesser General Public License for more
 * details.
 */

package com.nlu.entity.service.messaging;

import com.liferay.portal.kernel.messaging.BaseMessageListener;
import com.liferay.portal.kernel.messaging.Message;

import com.nlu.entity.service.ClpSerializer;
import com.nlu.entity.service.CpuLocalServiceUtil;
import com.nlu.entity.service.CpuServiceUtil;
import com.nlu.entity.service.HeapLocalServiceUtil;
import com.nlu.entity.service.HeapServiceUtil;
import com.nlu.entity.service.NetworkLocalServiceUtil;
import com.nlu.entity.service.NetworkServiceUtil;
import com.nlu.entity.service.NonHeapLocalServiceUtil;
import com.nlu.entity.service.NonHeapServiceUtil;
import com.nlu.entity.service.RamLocalServiceUtil;
import com.nlu.entity.service.RamServiceUtil;
import com.nlu.entity.service.SessionRequestLocalServiceUtil;
import com.nlu.entity.service.SessionRequestServiceUtil;

/**
 * @author NgocBao
 */
public class ClpMessageListener extends BaseMessageListener {
	public static String getServletContextName() {
		return ClpSerializer.getServletContextName();
	}

	@Override
	protected void doReceive(Message message) throws Exception {
		String command = message.getString("command");
		String servletContextName = message.getString("servletContextName");

		if (command.equals("undeploy") &&
				servletContextName.equals(getServletContextName())) {
			CpuLocalServiceUtil.clearService();

			CpuServiceUtil.clearService();
			HeapLocalServiceUtil.clearService();

			HeapServiceUtil.clearService();
			NetworkLocalServiceUtil.clearService();

			NetworkServiceUtil.clearService();
			NonHeapLocalServiceUtil.clearService();

			NonHeapServiceUtil.clearService();
			RamLocalServiceUtil.clearService();

			RamServiceUtil.clearService();
			SessionRequestLocalServiceUtil.clearService();

			SessionRequestServiceUtil.clearService();
		}
	}
}