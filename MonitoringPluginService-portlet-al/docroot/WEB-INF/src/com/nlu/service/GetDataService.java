...
public class GetDataService {
	...
	public AllReport getInfoAllReport(String date) throws IOException {
		...
		for (File f : fNetwork) {
			...
			for (String ct : listContent) {
				String[] listSubContent = ct.split(FileUtil.WHITE_SPACE);
				networkData.add(new DataPoints(Long.parseLong(listSubContent[0]),
						SystemUtil.convertByteToKB(Double
								.parseDouble(listSubContent[1])), SystemUtil
								.convertByteToKB(Double
										.parseDouble(listSubContent[2]))));
		...
		File[] fSessionRequest = getListFile(
				new File(fileUtil.pathFileData(FileUtil.PATH_SESSIONREQUEST)), dt);
		...
		for (File f : fSessionRequest) {
			...
			for (String ct : listContent) {
				String[] listSubContent = ct.split(FileUtil.WHITE_SPACE);
				sessionData.add(new DataPoints(Long.parseLong(listSubContent[0]),
						Double.parseDouble(listSubContent[1]), Double
								.parseDouble(listSubContent[2])));
	...
	public static void main(String[] args) throws IOException {
		...
		String content = fu
				.loadFile("C:\\Users\\thuthuy\\Desktop\\software_liferay\\sdk_server\\server\\monitor\\ram\\ram.2018-07-14.txt");
		Pattern patternMaxValue = Pattern.compile("(.*\\s+)(\\S+)\\s+max",
				Pattern.MULTILINE);
